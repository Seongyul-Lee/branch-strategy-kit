...
### 한 줄 요약
main 브랜치를 최신화한 뒤 `<type>/<name>` 형식의 새 브랜치를 만들고 체크아웃합니다. 인자를 생략하면 인터랙티브 모드로 진입합니다.

### Usage
```bash
./scripts/new-branch.sh                    # 인터랙티브 모드 (TTY 필요)
./scripts/new-branch.sh <type>             # type만 지정, name은 입력 프롬프트
./scripts/new-branch.sh <type> <name>      # 인자 모드 (CI/스크립트 호환)
```

### 인터랙티브 모드

인자 없이 실행하면 두 단계로 진행됩니다:

1. **type 선택** — 화살표 키(↑↓)로 type 목록을 이동하고 Enter로 확정
2. **name 입력** — `브랜치 이름을 입력하세요: ` 프롬프트에 직접 입력
...
> 인터랙티브 모드는 TTY 환경에서만 동작합니다. CI/파이프 환경에서는 인자 형식을 사용하세요.
> `Ctrl+C`로 중간에 빠져나오면 터미널 커서가 자동으로 복구됩니다.

### 인자
- `<type>` — 브랜치 타입. 다음 중 하나 (생략 시 인터랙티브 선택):
  - `feat` — 신규 기능
  - `fix` — 버그 수정
  ...
  - `chore` — 빌드/CI/설정 변경
- `<name>` — 브랜치 이름. 자동으로 kebab-case로 변환됨 (대문자→소문자, 공백/언더스코어→하이픈). 생략 시 입력 프롬프트가 뜹니다.
...
1. 인자 파싱 — 인자가 빠졌고 TTY이면 인터랙티브 입력 (방향키 type 선택 + name 프롬프트)
