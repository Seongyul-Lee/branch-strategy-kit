...
- **새로 `git clone`한 직후에는 반드시 `./scripts/bootstrap.sh`를 다시 실행하세요.**
  - Git alias(`git nb`/`fb`/`cleanup`)와 lefthook 훅은 `.git/config`와 `.git/hooks/`에만 등록됩니다.
  - 이 두 경로는 clone 시 새로 생성되므로, 이전 clone에서 bootstrap을 실행했어도 새 clone에는 적용되지 않습니다.
- `bootstrap.sh`는 **idempotent(멱등)** 합니다. 이미 설치·등록된 항목은 건너뛰므로 몇 번을 실행해도 안전합니다. "혹시 뭔가 빠졌나?" 싶을 때 그냥 다시 돌리면 됩니다.
- bootstrap을 건너뛴 상태에서 `git nb`/`git fb`가 `git: 'nb' is not a git command`로 실패한다면, 위 케이스입니다. `./scripts/bootstrap.sh`를 한 번 실행하면 복구됩니다.
...
<details>
<summary>발생 원인</summary>

workflow가 `pull_request` 트리거로 1회 이상 실행되어야 branch protection 설정 화면의 검색 결과에 status check이 등록됩니다.
...
패키지 매니저(특히 winget)로 설치 후 현재 셸의 PATH가 갱신되지 않았습니다.
...
gh가 설치되었지만 GitHub 인증이 되지 않았습니다.
...
관리자가 키트 도입 시 `.gitattributes`를 복사하지 않았습니다.
...
`.gitattributes` 파일은 있지만, `*.sh`, `*.yml`, `*.yaml`, `*.bash`에 대한 `eol=lf` 규칙이 빠져 있습니다.
...
로컬 main에 커밋되지 않은 변경이 있거나 머지 충돌이 발생했습니다.
...
`finish-branch.sh`는 `gh pr create --fill-first`를 사용합니다 — **첫 번째 커밋 메시지**를 PR 제목/본문으로 자동 채움. 커밋이 여러 개라면 두 번째 이후 커밋의 의도가 누락될 수 있습니다.
...
### `cleanup-merged.sh`에서 squash-merge된 브랜치가 정리되지 않음

<details>
<summary>발생 원인</summary>

`gh` CLI가 미설치이거나 미인증 상태입니다. gh CLI 없이도 일반 merge와 원격에서 삭제된 브랜치는 정리되지만, squash-merge된 브랜치는 GitHub PR 상태를 조회해야 감지할 수 있습니다.
...
`fzf`가 설치되어 있지 않습니다. 키트의 필수 의존성은 아니므로 fallback으로 번호 입력 모드가 동작합니다.
...
Windows의 `core.autocrlf=true`(기본값)와 `.gitattributes` 부재의 충돌입니다. 키트의 `.sh`/`.yml` 자산은 LF로 저장되어야 하는데(Linux GitHub Actions runner에서 실행), git이 working tree를 CRLF로 가정해서 불일치를 보고합니다.
...
Windows에서 `core.fileMode=false`(기본값)로 인해 git이 파일의 실행 권한을 working tree에 적용하지 않습니다.
...
### `git sync-main` 실행 시 "DEFAULT_BRANCH=develop 일 때만 사용할 수 있습니다" 에러

<details>
<summary>발생 원인</summary>

`.kit-config`의 `DEFAULT_BRANCH`가 `main`으로 설정되어 있습니다.
...
**정상 동작입니다.** 처음 clone 후 develop을 로컬에 체크아웃한 적이 없으면 자동으로 원격에서 가져옵니다.
...
관리자가 develop 브랜치를 아직 생성하지 않았습니다.
...
### `git nb` 실행 시 "인터랙티브 모드는 TTY가 필요합니다" 에러

<details>
<summary>발생 원인</summary>

CI·파이프 환경처럼 터미널이 없는 곳에서 인자 없이 `git nb`를 실행하면 인터랙티브 선택 메뉴를 표시할 수 없습니다.
